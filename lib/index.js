...
export default function slackin({
  ...
  if (params.theme) {
    if (params.theme in themes) {
      ...
      mainLog(`Specified theme (${theme}) not found, falling back to default`);
    }
  } else {
  ...
  if (params.accent) {
    theme.accent = tinycolor(params.accent).toHexString();
    mainLog(`Using a custom theme accent: ${params.accent}`);
  }
  ...
  if (params.proxy) {
    ...
    if (params.redirectFQDN) {
      app.use((req, res, next) => {
        if (req.headers['x-forwarded-proto'] === 'http') {
          res.redirect(`https://${params.redirectFQDN}${req.url}`);
        } else {
          next();
        }
      });
  ...
  app.get('/', (req, res) => {
    ...
    return res
      .type('html')
      .render('main', {
        coc, path, name, org, logo, channels, active, total, recaptcha, css, theme, analytics,
      });
  });
  ...
  app.get('/iframe/dialog', (req, res) => {
    ...
    res.type('html');
    res.render('main', {
      coc, path, name, org, channels, active, total, large, recaptcha, theme, analytics, iframe: true,
    });
  });
