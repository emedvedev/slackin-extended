...
function slackin({
  ...
  i18n.configure({
    ...
    directory: sysPath.join(__dirname, '/../locales'),
    objectNotation: true
  });
  ...
  app.post('/invite', json(), (req, res) => {
    ...
    if (channelsFiltered && !channelsFiltered.includes(channel)) {
      errorMessage = i18n.__('errors.notAPermittedChannel');
    } else if (channelsFiltered && !slack.getChannelId(channel)) {
      errorMessage = `${i18n.__('errors.channel')} "${channel}" ${i18n.__('errors.notFound')}`;
    } else if (!email) {
      errorMessage = i18n.__('errors.noEmail');
    } else if (recaptcha.secret && (!captchaResponse || !captchaResponse.length)) {
      errorMessage = i18n.__('errors.invalidCaptcha');
    } else if (!remail().test(email)) {
      errorMessage = i18n.__('errors.invalidEmail');
    } else if (emails && !match.any(email, acceptedEmails)) {
      errorMessage = i18n.__('errors.emailNotAccepted');
    } else if (coc && Number(req.body.coc) !== 1) {
      errorMessage = i18n.__('errors.agreeCoc');
    }
